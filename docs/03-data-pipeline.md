...
```
   ┌─────────────────────────────────────────┐
   │  Step 1: 拉取 Slack 历史消息             │
   │  · 用 Slack API 拉取目标频道全部消息      │
   │  · 含发言人 / 时间 / 文本                │
   └────────────────┬────────────────────────┘
                    │
   ┌────────────────▼────────────────────────┐
   │  Step 2: 筛选 Onboarding 阶段           │
   │  · 用规则（频道名 / 时间窗口 / 关键词）   │
   │    过滤出 Onboarding 阶段的对话          │
   │  · 排除已离开/已升级 Creator 的发言       │
   └────────────────┬────────────────────────┘
                    │
   ┌────────────────▼────────────────────────┐
   │  Step 3: 标注 & 准备                     │
   │  · 区分"提问消息" vs "回答消息"          │
   │  · 区分"Coach marker"（导师介入信号）    │
   │  · 准备给 LLM 的 batch 输入              │
   └────────────────┬────────────────────────┘
                    │
   ┌────────────────▼────────────────────────┐
   │  Step 4: LLM batch 分类（多轮迭代）      │
   │  · 用 LLM 给每条提问做意图分类            │
   │  · 多轮 prompt 迭代（v1 / v2 / v3）       │
   │  · 每轮基于上一轮结果改进 prompt          │
   └────────────────┬────────────────────────┘
                    │
   ┌────────────────▼────────────────────────┐
   ...
   │  · 把意图相同的问题聚成 FAQ              │
   │  · 统计每个 FAQ 的出现频次               │
   │  · 按 Coach × FAQ 二维分布看哪些团队问题严重 │
   └────────────────┬────────────────────────┘
                    │
   ┌────────────────▼────────────────────────┐
   │  Step 6: 输出 FAQ Dashboard              │
   │  · 可视化 FAQ 排名                       │
   │  · Coach Marker 分布                     │
   │  · 单条 FAQ 的真实历史对话案例           │
   └─────────────────────────────────────────┘
